...
class ThresholdMasker(Masker, metaclass=ABCMeta):
    ...
    def __init__(self, img_dir: str, out_dir: str, glint_thresholds: Iterable[float] = (0.875,),
...
class _MultiFileThresholdMasker(ThresholdMasker, metaclass=ABCMeta):
    ...
    def read_img(self, img_path: str) -> np.ndarray:
        ...
        """
        imgs = self.get_grouped_img_paths(img_path)
        return np.concatenate(*imgs, axis=2).astype(np.float)

    @property
    @abstractmethod
    def _file_matcher(self) -> re.Pattern:
        raise NotImplemented

    @staticmethod
    def get_grouped_img_paths(first_band_path: str) -> Tuple[str]:
        ...
        return tuple([str(Path(first_band_path).with_stem(f"{in_path_root}{i}.TIF")) for i in range(1, 6)])
...
class P4MSThresholdMasker(_MultiFileThresholdMasker):
    ...
    def get_mask_save_paths(self, in_path: str) -> Tuple[str]:
        ...
        return tuple([str(out_dir.joinpath(f"{in_path_root}{i}_mask.png")) for i in range(1, 6)])


class MicasenseRedEdgeThresholdMasker(ThresholdMasker):
